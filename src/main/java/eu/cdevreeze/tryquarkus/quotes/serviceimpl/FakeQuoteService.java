...
package eu.cdevreeze.tryquarkus.quotes.serviceimpl;

import com.google.common.collect.ImmutableList;
import com.google.common.collect.ImmutableMap;
import com.google.common.collect.ImmutableSet;
import eu.cdevreeze.tryquarkus.quotes.model.Quote;
import eu.cdevreeze.tryquarkus.quotes.model.QuoteBuilder;
...
/**
 * Fake quote service, using no database, but perfectly fit to test the web layer.
 *
...
public class FakeQuoteService implements QuoteService {
    ...
    private List<Quote> initialQuotes() {
        Validator validator = Objects.requireNonNull(validatorFactory).getValidator();
        return Stream.of(
                        new QuoteBuilder(
                ...
                .filter(qt -> validator.validate(qt).isEmpty())
                .map(QuoteBuilder::build)
                .collect(ImmutableList.toImmutableList());
    }

    private final AtomicReference<ImmutableMap<Integer, Quote>> quotesDb =
            new AtomicReference<>(
                    IntStream.range(0, initialQuotes().size())
                            .mapToObj(i -> Map.entry(i + 1, initialQuotes().get(i)))
